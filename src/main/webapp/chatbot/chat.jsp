...
%>
<div id="messageWindow2"  style="background-image:url('${pageContext.request.contextPath}/resources/img/hpimage2.jpg');width:300px;height:539px;overflow: auto;"></div>
<input id="inputMessage" type="text" onkeydown="if(event.keyCode==13){send();}" />
<input class="form-control" type="submit" value="전송" onclick="send();" />
<script type="text/javascript">
	//웹소켓 설정
	//'ws://아이피주소:8080/BoardCast.java 파일에 설정되있는 주소
	var webSocket = new WebSocket('ws://192.168.1.142:8080/broadcasting');
	...
	function onOpen(event) { //접속했을 때, 내 영역에 보이는 글.
			...
			document.getElementById('messageWindow2').appendChild(clear);
			//접속했을 때 접속자들에게 알릴 내용.
			 <%-- webSocket.send("<%=nick%>|\|상담원과 연결 중입니다. 잠시만 기다려주세요."); --%>
	}
